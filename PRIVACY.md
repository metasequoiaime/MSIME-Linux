...
**AI suggestions are disabled by default.** When enabled, the typed spelling and the surrounding composition context are sent to the OpenAI-compatible endpoint that the user configures (`deepseek`, `openai`, `siliconflow`, `groq` or a custom HTTPS endpoint). That third party's own privacy terms then apply to the request.
