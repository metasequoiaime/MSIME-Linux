...
The current desktop experience supports runtime switching among Quanpin, Shuangpin, Wubi and Japanese Romaji, a Chinese/direct-input toggle, mixed and dedicated English candidates, live candidates from local SQLite dictionaries, keyboard and mouse candidate selection, paging, Chinese/English punctuation, half/full-width input, configurable inline preedit, Quanpin/Shuangpin helpcodes, and persistent per-user settings.
...
- Set `mixed-english-candidates=true` to append up to five local English completions after Chinese candidates in Quanpin and Shuangpin. `mixed-english-minimum-prefix` controls the activation threshold and accepts 1–8; the defaults are disabled and 2.
...
| Mixed and dedicated English input | Supported | Engine ordering/failure/learning tests, controller/mapper/settings tests, generated `english.db`, transactional install and real D-Bus commit smoke |
| Mixed expressive candidates and extended input modes | Planned | Local extensions phase |
