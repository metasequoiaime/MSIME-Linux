...
4. **在线能力**：云候选、AI 联想、候选翻译和统一 provider 配置；所有调用异步且可降级。Linux 已完成并验证云候选、OpenAI-compatible AI provider、Secret Service 凭据隔离、DeepLX-compatible 翻译回退和 IBus generation/取消接线；翻译只改变显示元数据，不改变提交文本。
5. **桌面工具与发行**：设置应用、语音、剪贴板、手写、屏幕键盘、悬浮工具栏、Deb/RPM/Arch 包、升级迁移和诊断。Secret Service 凭据存储已作为在线能力的一部分落地；其余桌面工具与发行集成仍待后续阶段。
