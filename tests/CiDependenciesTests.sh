...
# The bootstrap may authenticate against api.github.com, whose host is a literal in the script,
# because the unauthenticated ceiling of 60 an hour is shared with every other customer on the
# runner's address. What it must never do is let that credential reach a URL built from
# .gitmodules. These three checks are what makes that true, so they replace an earlier blanket ban
# on the token appearing at all.
